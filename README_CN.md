...
- macOS: `Terminal Manager-1.2.1-arm64.dmg` 或 `Terminal Manager-1.2.1-x64.dmg`
...
- 默认 Shell（可从自动检测项中选择，也可输入自定义路径）
- 默认工作目录
- 字体大小（8-32）
- 滚动缓冲行数
- 主题（深色/浅色）

主题会即时生效；其他终端设置会用于后续新建终端，已打开的终端保持不变。
...
A: 按 `Ctrl+,` 打开设置，在"默认 Shell"中选择自动检测项或输入自定义路径并保存。也可以在资源管理器中的项目上点击右键，临时选择 Shell 并以该项目目录新建标签。
